...
router.get('/verify', isAuth, authController.getVerifyUser);
...
router.post('/login',
    [check('email').isEmail().withMessage("Please enter a valid email").normalizeEmail(),
     check('password','Please insert a valid password with numbers letters and minimum 8 characters').isLength({min:8}).matches(/^[A-Za-z0-9 .,'!&]+$/).trim()],  
    authController.postLogin);
router.post('/signup',[
...
//router.post('/check-reset-token', authController.postCheckResetToken);
//router.put('/new-password', authController.getReset);
//router.get('/request-password-reset', authController.postReset);
